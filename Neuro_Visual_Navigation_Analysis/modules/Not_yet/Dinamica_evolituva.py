...
def Condition_While(condition, size_windows, overlap):
    
    if condition != True:
        return int((condition*120 - size_windows)/overlap)
    else:
        condition = 61 # I put one more that the necessary for be sure that I consider all intervall
        return int((condition*120 - size_windows)/overlap)
        
def Main(dic, clu1, clu2, size_windows = 120, overlap = 2, condition_while = True):
    """
    condition_while : if "True" then analysis all trials, else [int] analysis first [int] trials
...
### This part it is only for see the progress percentaje 
    val = 0 
    total = 120*(condition_while-1)
    pequeno = overlap/ total*100
    
    condition = Condition_While(condition_while, size_windows, overlap)
      
    for i  in range(condition):
        
        try:
            print("{}%".format(val)) #See progress percenteje
            val += pequeno
            ...
            Save_images(autocor_1, autocor_2, cross_1, str(clu1), str(clu2), light_percentage, i)
...
def Save_images(auto1, auto2, corr1, clu1, clu2, light, num):
    font={"family" : "sans-serif", "color":"orangered"}
    nombre = ["Autocor_{}".format(clu1), "Autocor_{}".format(clu2), "Corr_cross_{}-{}".format(clu1,clu2)]
    l1 = round(light["l1"][num], 2) ; l2 = round(light["l2"][num], 2) ; l3 = round(light["l3"][num], 2) ; l4 = round(light["l4"][num], 2)
    d1 = round(light["d1"][num], 2) ; d2 = round(light["d2"][num], 2) ; d3 = round(light["d3"][num], 2) ; d4 = round(light["d4"][num], 2)
    label = "{} l1:{} l2:{} l3:{} l4:{} d1:{} d2:{} d3:{} d4:{}".format("{}",l1,l2,l3,l4,d1,d2,d3,d4)
    for val in np.array([nombre,[auto1,auto2,corr1]]).T:
        name = val[0]
        plt.clf()
        plt.imshow(val[1], cmap = "jet")
        plt.title(label.format(val[0]),fontdict = font)
        plt.savefig("/home/tomasg/Escritorio/Img/"+val[0]+"/{}".format(str(num)))
...
if __name__ == "__main__":
    ...
    Scor1=[]
    Score2=[]
    dic, dic_light = Main(Dicc_clu, 9, 10, 120, 2, condition_while = 4)
    fin=time.time()
